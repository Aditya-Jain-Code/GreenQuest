  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (userInfo?.email) {
        ...
        if (user?.role === "agent") {
          // 🎉 Redirect to Pickup Agent Dashboard
          localStorage.setItem("agentEmail", userInfo.email);
          router.push("/pickup");
          toast.success("Welcome, Pickup Agent!");
        } else {
          toast.error(
            "Access denied. Only pickup agents can access this page."
          );
    ...
    <div className="flex items-center justify-center min-h-screen">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-96 transform hover:scale-105 transition-transform duration-300">
        ...
        <p className="text-gray-600 text-center mb-6">
          Sign in using your Web3 wallet to access the pickup agent panel.
        </p>
